...
function main(currentTime) {
  ...
  val1 = localStorage.getItem(localhigh[0])
  if (val1 == null) {
    hs[0] = 0
  }
  else {
    hs[0] = JSON.parse(val1)
  }
...
function displaysaved() {
  ...
  console.log(typeof (linkelement))
  const leader = document.getElementsByClassName('actualLeaderboard')
  const text = document.createElement('div')
  text.innerHTML = 'LeaderBoard'
  document.getElementById('sidenav').appendChild(text)
  text.classList.add('navitems')
  let arr = new Array(5)
  for (let i = 0; i < 5; i++) {
    let val2 = localStorage.getItem(localhigh[i])
    if (val2 == null) {
      hs[i] = 0
    }
    else {
      hs[i] = JSON.parse(val2)
    }
    arr[i] = document.createElement('div')
    arr[i].innerHTML = `${hs[i]}`
    arr[i].classList.add('highitems')

    document.getElementById('sidenav').appendChild(arr[i])

  }
  // console.log(typeof(leader))
  // document.getElementById('sidenav').appendChild(leader)
}
